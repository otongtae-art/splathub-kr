...
[2026-04-25T00:46+09:00] [round 9] [deploying] Auto-capture mode (모바일 자이로) — 토글 ON 시 빈 섹터 진입할 때마다 자동 셔터 (800ms debounce). 셔터 버튼 emerald glow + pulse. 사용자는 물체 주변 걷기만 하면 됨 → 균등 분포 + UX 친화. R6 sector guidance + R7 sharpness + R8 toast 와 결합 — 입력 파이프라인 최종 자동화
