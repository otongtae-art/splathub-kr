...
[2026-04-25T03:50+09:00] [round 32] [deployed] VGGT 실패 에러 분류 + TRELLIS 폴백 — classifyVggtError() 가 quota/timeout/OOM/network/aborted 5종 분류, 각각 actionable advice 표시. 또한 R19 monster 폴백을 VGGT 완전 실패 케이스에도 적용 (1장 AI 시도 버튼). 사용자가 빈 손으로 가는 케이스 줄임. commit 957194b
[2026-04-25T03:58+09:00] [round 33] [deployed] Mobile viewer stats — 기존 'hidden sm:inline' 으로 mobile 에서 안 보이던 viewer 통계 (pts/평탄도) 를 mobile 에서도 약식 표시 (예: '12k·28%'). desktop 은 풀 라벨 유지. tooltip 으로 해석 안내. 모바일 사용자 (capture 주 채널) 가 결과 품질 즉시 인지. commit 6567d2f
[2026-04-25T04:06+09:00] [round 34] [deployed] 자동 학습 이동 옵션 — 토글 ON 시 30장 (TARGET_SHOTS) 도달하면 5초 countdown banner + 취소 버튼 → 자동 proceedToTraining(). R9 auto-capture 와 결합 시 사용자가 폰 들고 걷기만 하면 학습 페이지까지 hands-free 도달. commit 94f7eeb
[2026-04-25T04:14+09:00] [round 35] [deployed] ErrorBoundary for MeshViewer — components/ErrorBoundary.tsx generic class component, train 페이지 viewer 감싸 Three.js/WebGL 크래시 시 white-screen 방지. fallback UI 에 [.glb 다운로드만] [새로고침] 옵션 — 사용자가 결과 잃지 않게. commit 37c1028
[2026-04-25T04:22+09:00] [round 36] [deployed] ErrorBoundary 모든 viewer 사이트로 확장 — home (/), /m/[slug], /convert 의 ViewerShell/MeshViewer 모두 wrap. R35 패턴을 4개 viewer 전체에 적용. 어떤 페이지에서도 WebGL crash 시 white screen 안 남. commit 3d52f31
[2026-04-25T04:30+09:00] [round 37] [deployed] App-level error.tsx + R34 countdown 햅틱 — Next.js App Router 의 라우트 단위 error boundary 추가 ('다시 시도' / '홈으로'). R34 자동 학습 countdown 시작 시 warningHaptic() 더블탭 — 사용자가 화면 안 보고 있어도 인지. commit 68f20d9
[2026-04-25T04:38+09:00] [round 38] [deploying] PWA manifest — apps/web/app/manifest.ts (Next.js 14 라우트 핸들러). standalone display + portrait + start_url=/capture + 단축 액션 (촬영/예시). 모바일 사용자가 '홈 화면에 추가' 가능 → native app 같은 풀스크린 capture 경험
